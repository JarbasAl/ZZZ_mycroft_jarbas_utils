...
from mycroft_jarbas_utils.ssl.self_signed import create_self_signed_cert
...
class JarbasServer(object):
    def __init__(self):
        config = Configuration.get().get("server", {})
        self.host = config.get("host", "0.0.0.0")
        self.port = config.get("port", 5678)
        self.max_connections = config.get("max_connections", -1)
        self.address = u"wss://" + unicode(self.host) + u":" + unicode(
            self.port)
        ...
        self.factory = JarbasServerFactory(self.address)
        self.factory.protocol = JarbasServerProtocol
        ...
        if not exists(key) or not exists(cert):
            ...
            config["key_file"] = key
            self.factory.config_update({"jarbas_server": config}, True)
