...
class JarbasClientProtocol(WebSocketClientProtocol):
    ...
    def onConnect(self, response):
        ...
        self.factory.emitter.emit(Message("hivemind.connected",
    ...
    def onOpen(self):
        ...
        self.factory.emitter.emit(Message("hivemind.websocket.open"))
    ...
    def onMessage(self, payload, isBinary):
        ...
        self.factory.emitter.emit(Message("hivemind.message.received",
                                          data))

    def onClose(self, wasClean, code, reason):
        logger.info("WebSocket connection closed: {0}".format(reason))
        self.factory.emitter.emit(Message("hivemind.connection.closed",
                                          {"wasClean": wasClean,
                                           "reason": reason,
                                           "code": code}))
...
class JarbasClientFactory(WebSocketClientFactory, ReconnectingClientFactory):
    ...
    def register_internal_messages(self):
        self.emitter.on("hivemind.message.received",
                        self.handle_receive_server_message)
        self.emitter.on("hivemind.message.send",
    ...
    def handle_receive_server_message(self, message):
        ...
        if is_file:
            ...
            # forward hivemind message to internal bus
    ...
    def sendMessage(self, type, data, context=None):
        ...
        self.emitter.emit(Message("hivemind.message.sent",
                                  {"type": type,
                                   "data": data,
                                   "context": context,
                                   "raw": msg}))
